...
import gpusim.core.GpuResourceStatsComputer;
import gpusim.listeners.GpuEventInfo;
import gpusim.listeners.GpuUpdatesVgpusProcessingEventInfo;
import gpusim.provisioners.CoreProvisioner;
import gpusim.provisioners.GpuResourceProvisioner;
import gpusim.provisioners.GpuResourceProvisionerSimple;
import gpusim.schedulers.vgpu.VGpuScheduler;
import gpusim.schedulers.vgpu.VGpuSchedulerSpaceShared;
import gpusim.vgpu.VGpu;
import gpusim.vgpu.VGpuSimple;
import gpusim.vgpu.VGpuStateHistoryEntry;
import gpusim.videocards.Videocard;
import gpusim.videocards.VideocardSimple;
...
public class GpuSimple implements Gpu {
    ...
    private GpuSuitability createVGpuInternal(final VGpu vgpu) {
         ...
         * return new
         * HostSuitability("Just internal VMs inside a VmGroup can be created, not the VmGroup itself."
         * );
         * }
         */
    ...
    private GpuSuitability isSuitableForVGpu(final VGpu vgpu, final boolean inMigration,
         ...
         * logAllocationError(showFailureLog, vm, inMigration, "MB", this.getStorage(),
         * vm.getStorage());
    ...
    public final Gpu setActive(final boolean activate) {
        ...
        // final CloudSimTag tag = activate ? CloudSimTag.GPU_POWER_ON :
        // CloudSimTag.GPU_POWER_OFF;
        /*
         * final String msg = (activate ? "on" : "off") +
         * " (expected time: {} seconds).";
         * LOGGER.info("{}: {} is being powered " + msg, getSimulation().clockStr(),
         * this, delay);
    ...
    public boolean addMigratingInVGpu(final VGpu vgpu) {
         ...
         * which requires searching in such a list every time a VM is requested to be
         * migrated
    ...
    public String toString() {
        final String vc = videocard == null || Videocard.NULL.equals(videocard) ? ""
    ...
    public void enableUtilizationStats() {
        ...
        if (vgpuList.isEmpty()) {
            ...
            vgpuList.forEach(GpuResourceStatsComputer::enableUtilizationStats);
    }
     ...
     * 
     * if(powerModel.getHost() != null && powerModel.getHost() != Host.NULL &&
     * !this.equals(powerModel.getHost())){
     * throw new
     * IllegalStateException("The given PowerModel is already assigned to another Host. Each Host must have its own PowerModel instance."
     * );
     * }
     * 
    ...
    private double addVGpuResourceUseToHistoryIfNotMigratingIn(final VGpu vgpu,
        ...
        if (totalAllocatedMips + 0.1 < totalRequestedMips) {
            final String reason = getVGpusMigratingOut().contains(vgpu) ? "migration overhead"
                    : "capacity unavailability";
            final long notAllocatedMipsByCore = (long) ((totalRequestedMips - totalAllocatedMips)
