package gpusim.schedulers.gputask;
...
import gpusim.cloudlets.gputasks.GpuTaskExecution;
import gpusim.listeners.GpuTaskResourceAllocationFailEventInfo;
...
import static gpusim.listeners.GpuTaskResourceAllocationFailEventInfo.of;
...
public abstract class GpuTaskSchedulerAbstract implements GpuTaskScheduler {
    ...
    public <T extends GpuTask> List<T> getGpuTaskSubmittedList() {
        if (gpuTaskSubmittedList.isEmpty() && !enableGpuTaskSubmittedList) {
            LOGGER.warn(
                    "{}: The list of submitted GpuTask for {} is empty maybe because you didn't enabled it by calling enableGpuTaskSubmittedList().",
    ...
    private void updateVGpuResourceAbsoluteUtilization(final GpuTaskExecution gte,
        ...
        if (requested > available) {
            final String msg1 = available > 0 ? String.format("just %d was available", available)
                    : "no amount is available.";
            final String msg2 = vgpuResource.getClass() == Ram.class ? ". Using Virtual Memory," : ",";
            LOGGER.warn(
    ...
    private void updateOnResourceAllocationFailListeners(
        ...
        for (int i = resourceAllocationFailListeners.size() - 1; i >= 0; i--) {
            final EventListener<GpuTaskResourceAllocationFailEventInfo> listener = resourceAllocationFailListeners
                    .get(i);
     ...
     * }
     * 
     * private double diskTransferTime (final GpuTaskExecution gte, final Double
     * dataSize) {
     * return
     * gte.getGpuTask().getVm().getHost().getStorage().getTransferTime(dataSize.
     * intValue());
     * }
     */
    ...
    private int addGpuTasksToFinishedList() {
        final List<GpuTaskExecution> finishedGpuTasks = gpuTaskExecList.stream()
    ...
    protected double moveNextGpuTasksFromWaitingToExecList(final double currentTime) {
        ...
        while (!gpuTaskWaitingList.isEmpty() && optional.isPresent()) {
            ...
            final double estimatedFinishTime = optional
    ...
    private double getAbsoluteGpuTaskGpuUtilizationForAllCores(final double time,
        ...
        final double gpuTaskGpuUsageForOneCore = getAbsoluteGpuTaskResourceUtilization(
