...
import gpusim.listeners.VGpuGpuEventInfo;
import gpusim.listeners.VGpuVideocardEventInfo;
import gpusim.resources.Gpu;
import gpusim.resources.VGpuCore;
import gpusim.schedulers.gputask.GpuTaskScheduler;
import gpusim.schedulers.gputask.GpuTaskSchedulerTimeShared;
import gpusim.videocards.Videocard;
import gpusim.vms.GpuVm;
import gpusim.cloudlets.GpuCloudlet;
...
public class VGpuSimple implements VGpu {
    ...
    private double startTime;// can use from gpuvm or must be initialize wherever gpuvms starttime
                             // initialized
    ...
    public final VGpu setGddram(final long gddramCapacity) {
        if (this.isCreated()) {
            throw new UnsupportedOperationException(
                    "gddram capacity can just be changed when the vgpu was not created inside a videocard yet.");
        }
    ...
    public final VGpu setBw(final long bwCapacity) {
        if (this.isCreated()) {
            throw new UnsupportedOperationException(
                    "Bandwidth capacity can just be changed when the vgpu was not created inside a videocard yet.");
        }
     ...
     * if (this.isCreated()) {
     * throw new
     * UnsupportedOperationException("Storage size can just be changed when the Vm was not created inside a Host yet."
     * );
     * }
    ...
    public final VGpu setGpuTaskScheduler(final GpuTaskScheduler gpuTaskScheduler) {
        ...
        if (isCreated()) {
            throw new UnsupportedOperationException(
                    "GpuTaskScheduler can just be changed when the vgpu was not created inside a Videocard yet.");
        }
    ...
    public void updateMigrationStartListeners(final Gpu targetGpu) {
        ...
        for (int i = 0; i < onMigrationStartListeners.size(); i++) {
            final EventListener<VGpuGpuEventInfo> listener = onMigrationStartListeners.get(i);
    ...
    public void updateMigrationFinishListeners(final Gpu targetGpu) {
        ...
        for (int i = 0; i < onMigrationFinishListeners.size(); i++) {
            final EventListener<VGpuGpuEventInfo> listener = onMigrationFinishListeners.get(i);
            listener.update(VGpuGpuEventInfo.of(listener, this, targetGpu));
        }
    }
    ...
    /*
     * @Override
     * public boolean removeOnCreationFailureListener(final
     ...
     * final String desc = StringUtils.isBlank(description) ? "" :
    ...
    public void notifyOnGpuAllocationListeners() {
        ...
        for (int i = 0; i < onGpuAllocationListeners.size(); i++) {
            final EventListener<VGpuGpuEventInfo> listener = onGpuAllocationListeners.get(i);
    ...
    public void notifyOnGpuDeallocationListeners(final Gpu deallocatedGpu) {
        ...
        for (int i = 0; i < onGpuDeallocationListeners.size(); i++) {
            final EventListener<VGpuGpuEventInfo> listener = onGpuDeallocationListeners.get(i);
    ...
    public void notifyOnUpdateProcessingListeners() {
        ...
        for (int i = 0; i < onUpdateProcessingListeners.size(); i++) {
            final EventListener<VGpuGpuEventInfo> listener = onUpdateProcessingListeners.get(i);
            listener.update(VGpuGpuEventInfo.of(listener, this));
        }
    }
     ...
     * 
     * @Override
     * public final Vm setHorizontalScaling(final HorizontalVmScaling
     * horizontalScaling) throws IllegalArgumentException {
     * this.horizontalScaling = validateAndConfigureVmScaling(horizontalScaling);
     * return this;
     * }
     * 
     * @Override
     * public final Vm setRamVerticalScaling(final VerticalVmScaling
     * ramVerticalScaling) throws IllegalArgumentException {
     * this.ramVerticalScaling = validateAndConfigureVmScaling(ramVerticalScaling);
     * return this;
     * }
     * 
     * @Override
     * public final Vm setBwVerticalScaling(final VerticalVmScaling
     * bwVerticalScaling) throws IllegalArgumentException {
     * this.bwVerticalScaling = validateAndConfigureVmScaling(bwVerticalScaling);
     * return this;
     * }
     * 
     * @Override
     * public final Vm setPeVerticalScaling(final VerticalVmScaling
     * peVerticalScaling) throws IllegalArgumentException {
     ...
     * }
     * 
     * private <T extends VmScaling> T validateAndConfigureVmScaling (final T
     * vmScaling) {
     * requireNonNull(vmScaling);
     * if (vmScaling.getVm() != null && vmScaling.getVm() != Vm.NULL &&
     * vmScaling.getVm() != this) {
     ...
     * this.addOnUpdateProcessingListener(vmScaling::
     ...
     * if(cpuUtilizationStats == null || cpuUtilizationStats ==
     * VmResourceStats.NULL) {
     * this.cpuUtilizationStats = new VmResourceStats(this, vm ->
     * vm.getCpuPercentUtilization(getSimulation().clock()));
     * }
     * }
     */
