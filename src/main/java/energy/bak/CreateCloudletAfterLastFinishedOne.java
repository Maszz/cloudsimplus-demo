...
/**
 * An example showing how to dynamically create one Cloudlet after the previous
 * one finishes.
...
public class CreateCloudletAfterLastFinishedOne {
    ...
    private static final int HOST_PES_NUMBER = 64;
    private static final int VM_PES_NUMBER = 64;
    private static final int CLOUDLETS = VMS * VM_PES_NUMBER * 7;
    ...
    private void printEnergyConsumption() {
        for (Host host : datacenter.getHostList()) {
            ...
            double energy = (power * time) / (1000 * 3600); // Convert Watts*seconds to kWh

            System.out.printf("Host %d | Utilization: %.2f | Power: %.2f W | Energy: %.6f kWh%n",
                    host.getId(), utilization, power, energy);
        }
    }
    ...
    private Host createHost(int id) {
        ...
        Host host = (Host) new HostSimple(ram, bw, storage, peList)
                .setRamProvisioner(new ResourceProvisionerSimple())
                .setBwProvisioner(new ResourceProvisionerSimple())
                .setVmScheduler(new VmSchedulerSpaceShared())
                .setPowerModel(new PowerModelHostSimple(250, 175));
