...
public class Conference {
    ...
    public void run() {
        ...
        for (int i = 0; i < datacenters.size(); i++) {
            totalCloudletsGenerated.add(0);  // Initialize as {0,0,0,...}
        }
        createBrokersVmsAndCloudlets(datacentersConfig);
        // at t=0
        submitCloudlets(datacentersConfig);
        // every t=1
    ...
    private void submitCloudlets(JsonArray datacentersConfig) {
        int finish_count = 0;
        for (int i = 0; i < datacenters.size(); i++) {
            // from config file
            JsonObject dcConfig = config.getArray("DATACENTERS").get(i).getAsJsonObject();
            ...
            int toSubmit = Math.min(tps, lastCloudlets-currentCloudlets);
            if (toSubmit > 0) {
                List<Cloudlet> newCloudlets = createDynamicCloudlets(currentCloudlets, toSubmit,
                dcConfig);
                broker.submitCloudletList(newCloudlets);
                currentCloudlets += toSubmit;
                totalCloudletsGenerated.set(i, currentCloudlets);
                System.out.printf("🟢 %s: Submitted %d cloudlets at %.1f sec (Total: %d)\n",
                        broker.getName(), toSubmit, simulation.clock(), currentCloudlets);
            }
            if (totalCloudletsGenerated.get(i) >= lastCloudlets && getSuccessTask(broker) == lastCloudlets) {
                finish_count++;
            }            
        }
        if (finish_count == datacenters.size()) {
            simulation.terminate();
            System.out.printf("✅ All cloudlets completed at %.1f sec\n", simulation.clock());
        } else {
            finish_count=0;
        }
    }
    ...
    private long getSuccessTask(DatacenterBrokerSimple broker) {
